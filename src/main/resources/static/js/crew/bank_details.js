...
jQuery(document).ready(function () {
    ...
    if (action === 'modify') {
        ...
        fileInput.addEventListener("change", event => {
            ...
            Bank_Details.uploadPhoto(files[0]);
        });
    }

});

Bank_Details.update = function () {
    ...
    if (!isModified) {
        ...
        if (isValid) {
            ...
            Bank_Details.getModifiedFields(modifiedFields);

            var json = JSON.stringify(modifiedFields);
            var data = new FormData();
            data.append("modifiedFields", json);
            data.append("bankId", jQuery('#bankId').val());
            data.append("crewId", jQuery('#crewId').val());

            var ajaxCall = jQuery.ajax({
                url: jQuery('#contextPath').val() + "/crew/bank/update.ajax",
                method: 'POST',
                data: data,
                type: "POST",
...
Bank_Details.getModifiedFields = function (modifiedFields) {
    // Get dirty fields
