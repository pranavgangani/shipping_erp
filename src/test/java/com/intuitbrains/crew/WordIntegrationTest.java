...
import com.intuitbrains.service.common.WordDocumentGenerator;
...
public class WordIntegrationTest {
    static WordDocumentGenerator wordDocument;

    @BeforeClass
    public static void generateMSWordFile() throws Exception {
        WordIntegrationTest.wordDocument = new WordDocumentGenerator();
        wordDocument.handleSimpleDoc();
    }

    @Test
    public void whenParsingOutputDocument_thenCorrect() throws Exception {
        Path msWordPath = Paths.get(WordDocumentGenerator.output);
        ...
        assertEquals(wordDocument.convertTextFileToString(WordDocumentGenerator.paragraph1), paragraphs.get(4).getText());
        assertEquals(wordDocument.convertTextFileToString(WordDocumentGenerator.paragraph2), paragraphs.get(5).getText());
        assertEquals(wordDocument.convertTextFileToString(WordDocumentGenerator.paragraph3), paragraphs.get(6).getText());
    }
}